/*
in config.h it defines if master or slave board
...
Accelerometer (not being used nor defined)
SDA: PB9 (I2C2)
SCL: PB8 (I2C2)

Light sensors (L2 not being used nor defined, L1 being used as a test button)
L1: PA7
L2: PA11

Remote pins used as USART0 for controlling speed and steer with steerin device
PB6 (I2C1_SCL / USART0_TX)
PB7 (I2C1_SDA / USART0_RX)

board A master lado direito
board b Slave lado esquerdo
...
int16_t batteryState =0; 									// global variable for battery state to send to steer
...
void ShowBatteryState(uint32_t pin);						//Lights up back led in the pedals
void BeepsBackwards(FlagStatus beepsBackwards);	//Beeps in case of SPEED <-50
void ShutOff(void);															//Shutdown
